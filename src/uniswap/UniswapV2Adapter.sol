...
contract UniswapV2Adapter {
    ...
    function onFlashSwapExactTokensForTokensViaETH(uint256 _wethAmount, uint256 _amountOut, address[2] calldata _tokens, address _flasher, bytes calldata _data) internal {
        /// ███ Checks
        ...
        IERC20(weth).safeTransfer(tokenOutPair, wethAmount);
        ...
        // Step 6:
        // Call the flasher
        IFlashSwapper(flasher).onFlashSwapExactTokensForTokensViaETH(_amountOut, _data);
    ...
    function uniswapV2Call(address _sender, uint256 _amount0, uint256 _amount1, bytes memory _data) external {
        ...
        if (flashSwapType == FlashSwapType.FlashSwapExactTokensForTokensViaETH) {
            ...
            (uint256 amountOut, address tokenIn, address tokenOut, address flasher, bytes memory callData) = abi.decode(data, (uint256,address,address,address,bytes));
            onFlashSwapExactTokensForTokensViaETH(wethAmount, amountOut, [tokenIn, tokenOut], flasher, callData);
            return;
        }
    }
    ...
    function flashSwapExactTokensForTokensViaETH(uint256 _amountIn, uint256 _amountOutMin, address[2] calldata _tokens, bytes calldata _data) public {
        ...
        uint256 amountOut = IUniswapV2Router02(router).getAmountsOut(_amountIn, [_tokens[0], weth, _tokens[1]]);
        ...
        uint256 wethAmount = IUniswapV2Router02(router).getAmountsIn(_amount, [weth, amountOut])[0];

        // Step 2:
        // Borrow WETH from tokenIn/WETH liquidity pair (e.g. USDC/WETH)
        address token0 = IUniswapV2Pair(tokenInPair).token0();
        address token1 = IUniswapV2Pair(tokenInPair).token1();
        ...
        bytes memory data = abi.encode(FlashSwapTtype.FlashSwapExactTokensForTokensViaETH, abi.encode(amountOut, _tokens[0], _tokens[1], msg.sender, _data));

 ...
 * @dev FLTSinglePair but without leverage ratio range.
 */
interface IFLTNoRange {
    ...
    /**
     * @notice Event emitted when max supply updated
    ...
    /// @notice Error is raised if max supply invalid
    ...
    /**
     * @notice Update the max supply of FLT
     ...
     * -----------
     *
     * In order to incentives the rebalancing process, FLT offers zero-slippage
     * and low fees trade.
     *
     * pushc: Market Makers can swap collateral token to debt token.
     ...
     *        receive 2000 USDC minus 0.1% fee in exchange.
     *
     * pushd: Market Makers can swap debt token to collateral token.
     *        For example: suppose the gOHM price is 2000 USDC, when Fuse
     ...
     *        receive 1 gOHM minus 0.1% fee in exchange.
     *
     * Market price is determined using Rari Fuse Oracle Adapter.
     *
     * ------------
     * Maximum Swap Amount
     *
